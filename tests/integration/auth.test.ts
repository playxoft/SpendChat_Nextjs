...
describe("getCurrentUser", () => {
  ...
  it("normalises missing email/name to null", async () => {
    setSession({ id: "u2", email: null, name: null });
    expect(await getCurrentUser()).toEqual({ id: "u2", email: null, name: null });
  });
...
describe("ensureBootstrap", () => {
  it("seeds settings, 15 default categories, and a Personal profile", async () => {
    ...
    const [settings] = await getTestDb()
      .select()
      .from(userSettings)
      .where(eq(userSettings.userId, "u1"));
    expect(settings.currency).toBe("USD");

    const [profile] = await getTestDb()
      .select()
      .from(profiles)
      .where(eq(profiles.userId, "u1"));
...
describe("getUserSettings", () => {
  it("bootstraps settings on first read", async () => {
    const settings = await getUserSettings("fresh");
    expect(settings.userId).toBe("fresh");
