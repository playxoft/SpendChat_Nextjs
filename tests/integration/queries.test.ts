...
beforeEach(async () => {
  ...
  work = (await getProfiles(U)).find((p) => p.name === "Work")!.id;
...
describe("listTransactions", () => {
  it("returns all rows newest-first by date", async () => {
    const rows = await listTransactions(U);
  ...
  it("filters by type", async () => {
    const rows = await listTransactions(U, { type: "expense" });
    expect(rows).toHaveLength(3);
    expect(rows.every((r) => r.type === "expense")).toBe(true);
  });

  it("filters by date range", async () => {
    const rows = await listTransactions(U, { from: "2026-06-01", to: "2026-06-30" });
  ...
  it("filters by category and by profile", async () => {
    expect(await listTransactions(U, { categoryId: groceries })).toHaveLength(2);
    const workRows = await listTransactions(U, { profileId: work });
    expect(workRows.map((r) => r.title)).toEqual(["Tools"]);
  });

  it("searches title OR description (case-insensitive)", async () => {
    expect((await listTransactions(U, { search: "coffee" })).map((r) => r.title)).toEqual([
      "snacks",
    ]);
    expect((await listTransactions(U, { search: "veg" })).map((r) => r.title)).toEqual([
      "Veg apples",
    ]);
  });

  it("paginates with limit/offset", async () => {
    const page1 = await listTransactions(U, { limit: 2, offset: 0 });
    const page2 = await listTransactions(U, { limit: 2, offset: 2 });
...
describe("listTransactionsAsc", () => {
  it("returns oldest-first (reverse of listTransactions)", async () => {
    const rows = await listTransactionsAsc(U);
    expect(rows.map((r) => r.occurredOn)).toEqual([
      "2026-05-20",
      "2026-06-01",
      "2026-06-10",
...
describe("countTransactions", () => {
  it("counts with and without filters", async () => {
    expect(await countTransactions(U)).toBe(4);
    expect(await countTransactions(U, { type: "income" })).toBe(1);
  });
});

describe("getSummary", () => {
  it("totals income, expense, and balance", async () => {
    expect(await getSummary(U)).toEqual({
      income: 5000,
      expense: 3300,
      balance: 1700,
    });
...
describe("getCategoryBreakdown", () => {
  it("groups expenses by category, largest first (incl. uncategorized)", async () => {
    const rows = await getCategoryBreakdown(U, "expense");
  ...
  it("groups income by category", async () => {
    const rows = await getCategoryBreakdown(U, "income");
    expect(rows).toEqual([
...
describe("getMonthlyTrend", () => {
  it("buckets by month and type", async () => {
    const rows = await getMonthlyTrend(U, "2026-01-01");
  ...
  it("can be scoped to a single profile", async () => {
    const rows = await getMonthlyTrend(U, "2026-01-01", work);
...
describe("getCategories / getProfiles", () => {
  ...
  it("returns profiles in sidebar order", async () => {
    const profs = await getProfiles(U);
